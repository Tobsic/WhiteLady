...
class REST_CRUD_API {
  ...
  protected function checkLogin($db, $username, $password, $CRUDtype, $table){
		...
		if ($result->num_rows > 0) {
		  ...
		  while($row = $result->fetch_assoc()) {
					...
					if($success === true){
						...
						//TODO Do some checking like access to table retricted? user has rights to modify row? (location id and so on)
						return $success;
					}
		  }
		}
    ...
    $loginSuccess = $this->checkLogin($db, $username, $password, "create", $table);
    ...
    $loginSuccess = $this->checkLogin($db, $username, $password, "update", $table);
    if($loginSuccess === true){
    ...
    $loginSuccess = $this->checkLogin($db, $username, $password, "delete", $table);
